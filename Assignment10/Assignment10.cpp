...
// Status    : Complete
//
// This program will be used to assignment 10
//******************************************************************
/* 
*/
...
void readFile(Employee employee[])
{
	...
	if (infile.is_open())
	{
		...
		while (getline(infile, line))
		{
			...
			employee[i].setHourlyPay(strtod(line.substr(25,5).c_str(), NULL));
			employee[i].setNumDeps(atoi(line.substr(31,1).c_str()));
			employee[i].setType(atoi(line.substr(33,1).c_str()));
			i++;
		}
		infile.close();
	}
